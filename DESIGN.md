...
The system refuses the two ruts this category ships. It is not the generic InstantSearch results page (`full-version.png` — the explicit anti-reference): flat gray facets, a stock rating widget, undifferentiated rows. And it is not the AI-restaurant default of cream paper, high-contrast serif display, and a terracotta accent. Instead: a cool, high-legibility concourse ground; a bounded dark board reserved strictly for *live information*; one saturated enamel-blue signal that owns every interaction; and every number set in a tabular monospace so the whole surface aligns like a real board.
